...
class Form extends Model
{
    ...
    public $rules = [
        ...
        'code'                 => 'required|unique:renatio_formbuilder_forms|max:100',
        'template'             => 'required',
    ...
    public $hasMany = [
        ...
        'sections' => ['Renatio\FormBuilder\Models\Section', 'order' => 'sort_order']
    ];
        ...
        foreach ($this->fields as $field) {
            if ($field->validation_messages) {
                foreach ($field->validation_messages as $customMessage) {
                    ...
                    $messages[$key] = $customMessage['message'];
                }
     ...
     * @return array
     */
    private function iterateFields($key, $value, $fromPost = false)
    {
        $arr = [];
     ...
     * @return bool
     */
    private function checkForUploadFields($uploadType)
    {
        foreach ($this->fields as $field) {
     ...
     * @return mixed
     */
    private function getFormByCode($formCode)
    {
        return self::with([
